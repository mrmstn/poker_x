defmodule PokerX.TableTest do
  ...
  test "sitting and leaving", %{manager: manager} do
    ...
    assert Table.sit(manager, :player_two, @num_seats + 1) ==
             {:error, %{reason: :seat_unavailable}}

    assert Table.sit(manager, :player_two, 3) == :ok

    assert [%{seat: 1, id: :player_one}, %{seat: 3, id: :player_two}] =
             Table.get_state(manager).players
  ...
  test "buying in and cashing out", %{manager: manager} do
    PokerX.Bank.deposit(:player, 600)
    ...
    assert PokerX.Bank.balance(:player) == 600
  end
end
